...
router.post("/login", async (req, res) => {
  ...
  if (result) {
    return res.send({ message: result.details[0].message }).status(400);
    console.log("bad req");
  } //bad req
  ...
  res
    .header({ auth_token: access_token, refresh_token: refresh_token })
...
router.get("/renewAccessToken", async (req, res) => {
  ...
  try {
    ...
    return res
      .header({ auth_token: access_token })
      .send({ access_token, token_created: true })
      .status(200);
  } catch (error) {
    if (error === "JsonWebTokenError")
      ...
      //EXPIRED TOKEN
      res
        .send({ expired: true, message: "Session expired try sign in!" })
        .status(220);
    else res.status(400).send({ success: false, message: "Invalid token!" });
  }
});
...
router.get("/verification", async (req, res) => {
  ...
  try {
    ...
    if (error === "JsonWebTokenError")
      res.status(400).send({ success: false, message: "Unauthorized access!" });
    else if (isJwtExpired(refresh_token))
      //EXPIRED REFRESH TOKEN
      res
        .status(401)
        .send({ refreshtoken_expired: true, message: "Renew access token!" });
    else if (isJwtExpired(token))
      //EXPIRED REFRESH TOKEN
      res
        .status(202)
        .send({ accesstoken_expired: true, message: "Renew access token!" });
    else res.status(400).send({ success: false, message: "Invalid token!" });
  }
});
